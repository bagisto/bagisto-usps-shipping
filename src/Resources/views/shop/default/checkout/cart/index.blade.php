        ...
        <div class="w-full flex justify-between px-[60px] border border-t-0 border-b-[1px] border-l-0 border-r-0 py-[17px] max-lg:px-[30px] max-sm:px-[15px]">
            <div class="flex items-center gap-x-[54px] max-[1180px]:gap-x-[35px]">
                <a href="{{ route('shop.home.index') }}" class="flex min-h-[30px]" aria-label="Bagisto" >
                    <img src="{{ bagisto_asset('images/logo.svg') }}" alt="Bagisto " width="131" height="29" >
                </a>
            </div>
        </div>
    </div>
    ...
    @pushOnce('scripts')
        <script type="text/x-template" id="v-cart-template">
            ...
            <div>
                ...
                <!-- Cart Information -->
                <template v-else>
                    <div  class="flex flex-wrap gap-[75px] mt-[30px] pb-[30px] max-1060:flex-col" v-if="cart?.items?.length" >
                        <div class="grid gap-[25px] flex-1">
                            ...
                            <div class="flex justify-between items-center pb-[10px] border-b-[1px] border-[#E9E9E9] max-sm:block">
                                <div class="flex select-none items-center">
                                    <input type="checkbox" id="select-all" class="hidden peer" v-model="allSelected" @change="selectAll" >

                                    <label class="icon-uncheck text-[24px] text-navyBlue peer-checked:icon-check-box peer-checked:text-navyBlue cursor-pointer" for="select-all" >
                                    </label>
                                ...
                                <div  class="max-sm:ml-[35px] max-sm:mt-[10px]" v-if="selectedItemsCount" >
                                    <span class="text-[16px] text-[#0A49A7] cursor-pointer"  @click="removeSelectedItems" >
                                    ...
                                    @if (auth()->guard()->check())
                                        ...
                                        <span class="text-[16px] text-[#0A49A7] cursor-pointer"  @click="moveToWishlistSelectedItems" >
                            ...
                            <div  class="grid gap-y-[25px]"  v-for="item in cart?.items" >
                                <div class="flex gap-x-[10px] justify-between flex-wrap pb-[18px] border-b-[1px] border-[#E9E9E9]">
                                    <div class="flex gap-x-[20px]">
                                        <div class="select-none mt-[43px]">
                                            <input type="checkbox" :id="'item_' + item.id" class="hidden peer" v-model="item.selected" @change="updateAllSelected" >

                                            <label class="icon-uncheck text-[24px] text-navyBlue peer-checked:icon-check-box peer-checked:text-navyBlue cursor-pointer" :for="'item_' + item.id" ></label>
                                        </div>

                                        <!-- Cart Item Image -->
                                        <x-shop::media.images.lazy class="h-[110px] min-w-[110px] max-w[110px] rounded-[12px]" ::src="item.base_image.small_image_url" ::alt="item.name" width="110" height="110" ::key="item.id" ::index="item.id">
                                        </x-shop::media.images.lazy>

                                        <!-- Cart Item Options Container -->
                                        <div class="grid place-content-start gap-y-[10px]">

                                            <p class="text-[16px] font-medium" v-text="item.name">
                                            </p>
                                            
                                            @if (core()->getConfigData('sales.carriers.usps.active')) 
                                            <div  v-if="item.type == 'virtual' || item.type == 'downloadable' || item.type == 'booking'" class="row col-12 no-padding no-margin item-price">
                                                <div class="product-price">
                                                    <span class="text-[14px]" style="color:red;">{{ __('usps::app.admin.system.shipping-not-available')}}</span>
                                                </div>
                                            </div>
                                            @endif

                                            <!-- Cart Item Options Container -->
                                            <div class="grid gap-x-[10px] gap-y-[6px] select-none" v-if="item.options.length" >
                                                <!-- Details Toggler -->
                                                <div class="">
                                                    <p class="flex gap-x-[15px] text-[16px] items-center cursor-pointer whitespace-nowrap" @click="item.option_show = ! item.option_show" >
                                                        @lang('shop::app.checkout.cart.index.see-details')

                                                        <span class="text-[24px]" :class="{'icon-arrow-up': item.option_show, 'icon-arrow-down': ! item.option_show}" ></span>
                                                    </p>
                                                </div>
                                            ...
                                            <div class="sm:hidden">
                                                <p class="text-[18px] font-semibold" v-text="item.formatted_total">
                                                </p>
                                                
                                                <span class="text-[16px] text-[#0A49A7] cursor-pointer"  @click="removeItem(item.id)" >
                                                    @lang('shop::app.checkout.cart.index.remove')
                                                </span>
                                            </div>

                                            <x-shop::quantity-changer name="quantity" ::value="item?.quantity" class="flex gap-x-[10px] border rounded-[54px] border-navyBlue py-[5px] px-[14px] items-center max-w-max" @change="setItemQuantity(item.id, $event)" >
                            ...
                            <div class="flex flex-wrap gap-[30px] justify-end">
                                <a class="secondary-button max-h-[55px] rounded-[18px]" href="{{ route('shop.home.index') }}" >
                                    @lang('shop::app.checkout.cart.index.continue-shopping')
                                </a> 

                                <button class="secondary-button max-h-[55px] rounded-[18px]" @click="update()" >
                    ...
                    <!-- Empty Cart Section -->
                    <div class="grid items-center justify-items-center w-[100%] m-auto h-[476px] place-content-center text-center" v-else >
