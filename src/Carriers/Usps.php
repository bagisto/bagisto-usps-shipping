...
class Usps extends AbstractShipping
{
    ...
    public function calculate()
    {
        ...
        $shippingHelper     = app('Webkul\UspsShipping\Helpers\ShippingMethodHelper');
        $getCommonServices  = app('Webkul\UspsShipping\Repositories\UspsRepository');
        ...
        if ( isset($data['response']) && $data['response'] && !$data['errorResponse'] && !empty($serviceData))
        {
            foreach ($serviceData as $key => $uspsServices) {
                ...
                $object->carrier            = 'usps';
                $object->carrier_title      = $uspsMethod;
                ...
                if (! is_array($shippingRate)) {
                    $shippingRates['usps'] = $rates;
                    session()->put('shipping_rates', $shippingRates);
                } else {
                    session()->put('shipping_rates.usps', $rates);
                }
            ...
            if ( $data['response'] ) {
                foreach ($data['response'] as $services) {
                    $totalShippingCost  = 0;
                    $rate               = 0;
                    $classId            = '';
                    ...
                    $shippingRates = session()->get('shipping_rates');
